...
4. Перед commit выполни короткий self-review текущего diff в этой же сессии. После каждого нового head дождись exact-head `quality`, `windows-ssl-regression` и aggregate status `checks`; evidence старого head не переносится на новый.
5. Соблюдай failure budget: максимум 3 code-changing CI-fix cycles и 0 scope expansion на task; flaky retry не считай fix cycle только при evidence и без изменения кода.
...
7. YELLOW можно довести до green PR, но не merge и не запускать зависимые задачи без human/risk decision. GREEN merge возможен только после deterministic exact-head checks, aggregate `checks`, resolved existing GitHub threads и всех остальных merge gates.
