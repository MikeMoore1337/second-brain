...
it("renders exactly four semantic directions and removes the old decorative cards", async () => {
  ...
  expect(semanticGroups.map((group) => group.tools.length)).toEqual([5, 4, 4, 6]);
  ...
  expect(rendered.querySelectorAll(".semantic-tool-link")).toHaveLength(19);
  expect(rendered.querySelectorAll(".semantic-functional-surface")).toHaveLength(19);
  expect(rendered.querySelectorAll(".semantic-functional-surface[hidden]")).toHaveLength(19);
...
it("keeps one compact navigation entry per tool and activates its mounted surface", async () => {
  ...
  expect(new Set(links.map((link) => link.dataset.semanticToolLink)).size).toBe(19);
