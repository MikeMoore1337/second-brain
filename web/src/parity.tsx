      ...
      <div className="entry-copy">
        ...
        {!draftResponse ? <div className="capture-panel" data-capture-panel aria-busy={busy}>
          <div className="mode-switch" role="group" aria-label="Режим добавления">
            {(["url", "text", "voice"] as const).map((item) => <button className={`mode-button${mode === item ? " is-active" : ""}`} key={item} type="button" disabled={busy || microphonePending} aria-pressed={mode === item} onClick={() => { if (item !== "voice") clearAudioState(); setMode(item); setError(""); }}>{item === "url" ? "URL" : item === "text" ? "Текст" : "Голос"}</button>)}
          </div>
