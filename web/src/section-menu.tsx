...
export function SectionMenu({ groups = semanticGroups }: SectionMenuProps) {
  ...
  return <div className="section-menu" ref={root}>
    ...
    {open ? <div id="section-dropdown" className="section-dropdown">
      ...
      <nav className="section-menu-groups" aria-label="Все направления">{groups.map((group) => { const groupTarget = `#semantic-group-${group.id}`; const count = group.tools.length as number; return <div className="section-menu-group" key={group.id}><a className="section-dropdown-group-link" href={groupTarget} onClick={() => focusDestination(groupTarget, () => setOpen(false))}><Icon name={group.icon} size={28} /><span className="section-dropdown-group-copy"><strong>{group.label}</strong><small>{count} {count === 1 ? "инструмент" : count < 5 ? "инструмента" : "инструментов"}</small></span></a></div>; })}</nav>
