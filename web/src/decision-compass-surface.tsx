...
const RELATION_LABELS: Record<string, string> = {
  simulate_advisor_same_option: "Смоделированный выбор и совет указали один и тот же ID",
  simulate_advisor_different_options: "Смоделированный выбор и совет указали разные ID",
  simulate_advisor_not_comparable: "Смоделированный выбор и совет нельзя сопоставить",
  simulate_behavior_same_option: "Смоделированный выбор и поведение указали один и тот же ID",
  simulate_behavior_different_options: "Смоделированный выбор и поведение указали разные ID",
  simulate_behavior_not_comparable: "Смоделированный выбор и поведение нельзя сопоставить",
  advisor_behavior_same_option: "Совет и поведение указали один и тот же ID",
  advisor_behavior_different_options: "Совет и поведение указали разные ID",
  advisor_behavior_not_comparable: "Совет и поведение нельзя сопоставить",
  behavior_binding_missing: "Для поведенческого сравнения не задана точная связка",
...
export function DecisionCompassSurface(): ReactElement {
  ...
  const [status, setStatus] = useState("Данные компаса решения загружаются только после явного действия.");
  ...
  function bindBehaviorOption(): void {
    ...
    setStatus("Точная связка варианта и наблюдаемого контекста задана явно.");
  }
  ...
  function currentRequest(): DecisionCompassRequest | null {
    ...
    if (binding && (!scopeFingerprint || !options.some((option) => option.id === binding.request_option_id))) {
      setError("Точная связка устарела; выбери поведенческий контекст и вариант заново.");
      return null;
    }
    return makeRequest(
      goals,
  ...
  async function buildCompass(): Promise<void> {
    ...
    try {
      ...
      setStatus("Компас решения построен без обращения к провайдеру.");
    } catch (caught) {
      setError(safeError(caught, "Не удалось построить компас решения."));
      setStatus("Компас решения не построен.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function previewAdvisor(): Promise<void> {
    const request = currentRequest();
    if (!request || !compass) {
      if (!compass) setError("Сначала построй компас решения без провайдера.");
      return;
    }
  ...
  function renderSimulate(branch: DecisionCompassSimulateBranch): ReactElement {
    ...
    return (
      <section className="decision-compass-panel decision-compass-branch" aria-labelledby="decision-compass-simulate-title">
        <div className="decision-compass-panel-heading"><span>04</span><div><h4 id="decision-compass-simulate-title">Вероятный выбор</h4><p>Смоделировать меня показывает прогноз вероятного выбора, а не рекомендацию.</p></div></div>
        <p className="decision-compass-state">{stateLabel(branch.state, SIMULATE_STATE_LABELS, "Вероятный выбор не определён")}</p>
        {option?.id ? <p className="decision-compass-emphasis">ID вероятного выбора: <code>{option.id}</code></p> : null}
  ...
  function renderBehavior(branch: DecisionCompassBehaviorBranch): ReactElement {
    ...
    return (
      <section className="decision-compass-panel decision-compass-branch" aria-labelledby="decision-compass-behavior-title">
        <div className="decision-compass-panel-heading"><span>05</span><div><h4 id="decision-compass-behavior-title">Наблюдаемое поведение</h4><p>Показывается только выбранный владельцем точный контекст; автосопоставления нет.</p></div></div>
    ...
    return (
      <div className="decision-compass-result-list">
        ...
        <section className="decision-compass-panel decision-compass-branch" aria-labelledby="decision-compass-growth-title">
          <div className="decision-compass-panel-heading"><span>06</span><div><h4 id="decision-compass-growth-title">Связь поведения с целью</h4><p>Это существующая проверенная связь роста, не оценка варианта и не причинный вывод.</p></div></div>
        ...
        <section className="decision-compass-panel decision-compass-relations" aria-labelledby="decision-compass-relations-title">
          <div className="decision-compass-panel-heading"><span>09</span><div><h4 id="decision-compass-relations-title">Структурные связи</h4><p>Только механические отношения по ID между ветками; каждая ветка остаётся самостоятельной.</p></div></div>
        ...
        <details className="decision-compass-technical" aria-label="Техническая информация о компасе решения">
  ...
  return (
    <section id="decision-compass" className="decision-compass-surface" aria-labelledby="decision-compass-title" aria-busy={busy !== null} data-decision-compass-surface>
      ...
      <section className="decision-compass-panel" aria-labelledby="decision-compass-behavior-input-title">
        <div className="decision-compass-panel-heading"><span>03</span><div><h4 id="decision-compass-behavior-input-title">Необязательный наблюдаемый контекст</h4><p>Загрузи контексты отдельным действием и выбери точный отпечаток вручную. Даже единственный контекст не выбирается автоматически.</p></div></div>
        ...
        <label className="decision-compass-label" htmlFor="decision-compass-cohort">Точный отпечаток контекста</label>
        ...
        {cohortPatterns.length ? <label className="decision-compass-label" htmlFor="decision-compass-cohort-select">Или выбери один загруженный точный контекст<select className="decision-compass-input" id="decision-compass-cohort-select" value={scopeFingerprint} disabled={busy !== null} onChange={(event) => chooseScope(event.target.value)}><option value="">Не выбран</option>{cohortPatterns.map((pattern, index) => <option value={pattern.cohort?.cohort_fingerprint} key={pattern.cohort?.cohort_fingerprint}>Контекст {index + 1} · {safeText(pattern.cohort?.domain, "домен не указан")}</option>)}</select></label> : null}
        {selectedPattern?.selected_option ? <div className="decision-compass-binding"><p className="decision-compass-muted">Доступный наблюдаемый вариант показывается только как точный идентификатор: индекс {selectedPattern.selected_option.option_index}, отпечаток <code>{selectedPattern.selected_option.option_fingerprint}</code>.</p><label className="decision-compass-label" htmlFor="decision-compass-binding-option">Связать с вариантом этого запроса<select className="decision-compass-input" id="decision-compass-binding-option" value={bindingOptionId} disabled={busy !== null} onChange={(event) => setBindingOptionId(event.target.value)}><option value="">Не связывать</option>{options.filter((option) => option.label.trim()).map((option) => <option value={option.id} key={option.id}>{option.id}</option>)}</select></label><button className="decision-compass-button decision-compass-button-quiet" type="button" disabled={!bindingOptionId || busy !== null} onClick={bindBehaviorOption}>{binding ? "Обновить точную связку" : "Задать точную связку"}</button></div> : null}
        {binding ? <p className="decision-compass-inline-success" role="status">Точная связка задана для ID <code>{binding.request_option_id}</code>. Равенство подписей не используется.</p> : null}
      </section>
      <div className="decision-compass-build-row"><button className="decision-compass-button decision-compass-button-primary" type="button" disabled={busy !== null || !selectedGoalUuid} aria-busy={busy === "build"} onClick={() => void buildCompass()}>{busy === "build" ? "Строю компас…" : "Смоделировать меня"}</button><label className="decision-compass-date-label" htmlFor="decision-compass-as-of">Срез прогресса<input className="decision-compass-input" id="decision-compass-as-of" type="datetime-local" value={progressAsOf.slice(0, 16)} disabled={busy !== null} onChange={(event) => setProgressAsOf(event.target.value ? new Date(event.target.value).toISOString() : nowIso())} /></label></div>
      {compass ? <section className="decision-compass-result" aria-labelledby="decision-compass-result-title"><div className="decision-compass-result-heading"><div><p className="decision-compass-eyebrow">Проверенный срез</p><h4 id="decision-compass-result-title">Результаты по выбранной цели</h4></div><p>Собрано: {safeText(compass.provenance.progress_as_of)}</p></div><p className="decision-compass-freshness">Цель перепроверена по точному UUID и отпечатку личности перед выдачей результата.</p>{renderCompassResult(compass)}</section> : null}
    </section>
  );
}
