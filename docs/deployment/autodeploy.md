...
pipeline запускается заново: `uv sync --locked`, exact Python 3.14 bytecode refresh,
`doctor`, `vault validate`,
...
содержимое файлов не читается и найденные paths не выводятся. Разрешён только
узкий generated-state contract:

- `.venv/**` — первым шагом полного rerun выполняется `uv sync --locked --python
  3.14`, после чего validation использует reconciled project environment;
- `web/node_modules/**` — `npm ci` удаляет и пересоздаёт dependency tree из
  tracked `package-lock.json` до check/build;
...
Любой другой ignored path, включая root/web `.env`, `.env.*`, `*.env`,
credential/key/certificate-like file, arbitrary config, cache или пользовательский
файл, даёт `STOP / HUMAN_REQUIRED`. Даже secret-shaped `.env` и key/certificate
paths внутри generated roots не входят в allowlist. Helper не удаляет и не
перезаписывает неизвестное ignored state.
