...
Chevron у обычных одновыборных native `<select>` относится к браузерной
индикации формы, а не к растровой семье из 68 файлов. Он задан отдельным
same-origin SVG `web/public/icons/select-chevron.svg` в signal-фиолетовой
палитре; Unicode, emoji и `data:`-изображения для этой индикации запрещены.
В `forced-colors` используется системная стрелка браузера.
...
| Выполнение и обратная связь | Галочка внутри возвратной стрелки — `execution-feedback` |
...
## Обязательное правило уникальности

Разным разделам запрещено назначать одинаковую иконку или алиасы одного
изображения. Меню и заголовок одного раздела используют общий канонический
символ; повторение одинаковых служебных действий допустимо.
Если уникального подходящего символа нет, рисуется новый в существующей семье.
Перед выпуском владелец явно одобряет рисунок, показанный в реальном размере
и в контексте интерфейса. До одобрения это только локальный кандидат.

## Компас решения: локальный кандидат на согласование

`decision-compass` — круглый аметистовый стеклянный компас с диагональной
двухконечной стрелкой и четырьмя отметками. Подключён в меню и заголовке.
Встроенный image_gen, 15 сентября 2026; референс материала —
`relation-detail-192.webp`. Исходник: `exec-70f3c431-ce93-4099-acde-d9397a533af9.png`.
Настоящая alpha; sharp меняет только размер и формат (48/96/192 px,
WebP quality 84, alphaQuality 100). Новых runtime-зависимостей нет.
Статус: ожидает явного визуального одобрения владельца перед выпуском.

Запрос: один узнаваемый круглый компас из прозрачного тёмного аметистового
стекла, диагональная двухконечная стрелка на северо-восток, четыре отметки,
лавандовые фаски, свет сверху слева, ракурс три четверти. Без букв, документов,
моста, циферблата часов, пола, внешнего свечения и нарисованной шахматной сетки;
квадратный PNG с прозрачным фоном, читаемый при 40 px.

## Личное планирование: одобренная иконка

`personal-planning` — календарь с двумя верхними креплениями и шестью утопленными
ячейками из тёмного аметистового стекла. Левая боковая грань видна в три четверти,
поэтому символ сохраняет общий ракурс существующей семьи и не повторяет три
последовательно связанные панели `timeline`. Один канонический символ используется
в карточке и заголовке раздела «Личное планирование»; вариант показан владельцу
отдельно на прозрачном фоне и явно одобрен 16 сентября 2026 года.

Исходник создан встроенным `image_gen` по референсам `timeline-detail-192.webp`
и `growth-detail-192.webp`: `exec-9f65278e-b73a-44dd-8e62-f99dff710c9c.png`,
SHA-256 `57ad1f775b58c60e93f6e40a985d4f1da725f9ba84975170ae20e818b9e68a09`.
Изображение обработано только alpha-trim, приведением к 86% полю и экспортом
в локальные WebP 48/96/192 px, без новой runtime-зависимости:

- `personal-planning-compact-48.webp` — 1134 байта, SHA-256 `ce3453809e0d572708b95797b7db6f9cb5367a3d5c4c8bfeba65076773cf922c`;
- `personal-planning-compact-96.webp` — 2574 байта, SHA-256 `21de7c3ed3a74cadd78cc27632ab8d8f57946b2a823260f125405eaf7c6953f4`;
- `personal-planning-detail-192.webp` — 6246 байт, SHA-256 `353ef6b864c3bca39568db118bcbff91720319953b1074568c1de7c43f77c559`.

## Адаптивный профиль: одобренная иконка

`adaptive-profile` — цельная панель настройки из тёмного аметистового стекла
с тремя утопленными вертикальными каналами и квадратными ползунками на разной
высоте. Один канонический символ используется в карточке и заголовке раздела
«Адаптивный профиль»; круглая служебная стрелка `refresh` там больше не
применяется. Визуальный вариант показан владельцу в контексте интерфейса и
явно одобрен 16 сентября 2026 года.

Исходник создан встроенным `image_gen` по референсам `diagnostics-detail-192.webp`
и `growth-detail-192.webp`: `exec-0736d978-48ba-43f6-81a7-a2bd9f80f39f.png`,
SHA-256 `084a821c61549124c21ceddacbd12b91203100078362a03f6fcebd1dee88e442cc1b`.
Изображение обработано только alpha-trim, приведением к 86% полю и экспортом
в локальные WebP 48/96/192 px, без новой runtime-зависимости:

- `adaptive-profile-compact-48.webp` — 1208 байт, SHA-256 `cc3a10b73f03e077ddebc8721e54646fb776838905ac43bda24bdbf2612d8a30`;
- `adaptive-profile-compact-96.webp` — 2590 байт, SHA-256 `60749c03078d57e3f535352dee4d70fcd9e44eb2c60ba4aeed3ee8b20bcf789b`;
- `adaptive-profile-detail-192.webp` — 6324 байта, SHA-256 `c3c01f138aa6a204208ff2bc78388c2a73275eb9d1d1685b1671f12521eb2d69`.

## Выполнение и обратная связь: одобренная иконка

`execution-feedback` — объёмная галочка внутри незамкнутой возвратной стрелки.
Галочка обозначает зафиксированное выполнение, а стрелка — сравнение результата
с намерением и последующую обратную связь. Левая боковая грань видна в три
четверти; прозрачное аметистовое стекло и лавандовые фаски продолжают общий
материал семейства. Один канонический символ используется в карточке и
заголовке раздела «Выполнение и обратная связь»; вариант показан отдельно на
прозрачном фоне и явно одобрен владельцем 16 сентября 2026 года.

Исходник создан встроенным `image_gen`: `exec-033236b3-a21e-4503-91a0-3f707d301821.png`,
SHA-256 `25010483f135535e0773325f9f9ac59025b4e1104dd625dc9828add99ec4e765`.
Изображение обработано только alpha-trim, приведением к 86% полю и экспортом
в локальные WebP 48/96/192 px, без новой runtime-зависимости:

- `execution-feedback-compact-48.webp` — 2016 байт, SHA-256 `c98851c47998aeac529060362cfceb669fc38fb2093c19e4b83573a105138a5a`;
- `execution-feedback-compact-96.webp` — 4752 байта, SHA-256 `8c5daf3bbb38725de9eadbd1826db905aa56c35b2cc1e537b1d8d357c4463264`;
- `execution-feedback-detail-192.webp` — 10766 байт, SHA-256 `48776b109cf08f38e30158d1179404919dc3062c3a46e92db8f6c23f66fb4ec5`.
